...
% DMD added fix to deal with "." in dir name
...
for i=1:length(folders)
    ...
    [pth,fname,ext] = fileparts(folders{i});            %This causes an issue when the dir contains a '.'
    if ~isempty(ext) &&  ~isempty(regexp(folders{i},'\.', 'once'))%determine if folder contains folder with '.'
        if isdir([fname,ext])
            fname = [fname,ext]; 
        %else
