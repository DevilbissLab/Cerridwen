...
function hdr = graph(fid, file_version)
   ...
   %  Struct						% off + size
   unused = fread(fid, 1, 'int16')';			% 0 + 2             main HDR length
   hdr.file_version = fread(fid, 1, 'int32');		% 2 + 4         
   hdr.ext_item_header_len = fread(fid, 1, 'int32');	% 6 + 4
   hdr.num_channels = fread(fid, 1, 'int16');		% 10 + 2        num_channels
   hdr.horiz_axis_type = fread(fid, 1, 'int16');	% 12 + 2
   hdr.curr_channel = fread(fid, 1, 'int16');		% 14 + 2
   hdr.sample_time = fread(fid, 1, 'double');		% 16 + 8         (msec/sample) 
   hdr.time_offset = fread(fid, 1, 'double');		% 24 + 8         (msecs)
...
function hdr = per_chan_data(fid, file_version)
   ...
   %  Struct						% off + size
   hdr.chan_header_len = fread(fid, 1, 'int32')';	% 0 + 4             Length of channel header 
   ...
   hdr.volt_offset = fread(fid, 1, 'double')';		% 52 + 8             ampl offset (units) 
   hdr.volt_scale = fread(fid, 1, 'double')';		% 60 + 8        ampl scale (units/div)
   hdr.units_text = deblank(fread(fid, 20, '*char')');	% 68 + 20
   hdr.buf_length = fread(fid, 1, 'int32')';		% 88 + 4            samples 
