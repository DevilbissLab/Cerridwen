function [dataStruct, status] = Spike2DataLoader(params)
...
try
    ...
    %Add Critrical fields to Struct
    dataStruct.SubjectID = '';
    dataStruct.Comment = '';
    dataStruct.StartDate = [];
    dataStruct.FileFormat = '.smr';
    dataStruct.Hz = []; % the issue shere is that each channel can have its own sample rate
    ...
    for n = 1:length(ChannelNames)
        if ChannelNames(n).kind == 1 %if data channel
            ...
            if any(strcmpi(params.DataChannels,'ALL')) ||  any(~cellfun(@isempty,ChanCellArray))
                ...
                if dataIDX == 1
                    dataStruct.FileFormat = ChannelHeader.system; %Assign 1st data channel file format
                    dataStruct.Hz = ChannelHeader.sampleinterval;
                    dataStruct.StartDate = datenum([0 0 0 0 0 ChannelHeader.start]); %smr Does not store real time
                end
        ...
        elseif any(strcmpi(params.EventChannel,'ALL')) || strcmpi(ChannelNames(n).title,params.EventChannel) % if there is a keyboard / digimark channel
            ...
            disp(['Channel: ',ChannelNames(n).title, ' does not contain data.']);   
            end
        end
    end
