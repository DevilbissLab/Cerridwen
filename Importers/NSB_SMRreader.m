function [dataStruct, status] = NSB_SMRreader(params)
...
try
    ...
    FileHeader=SONFileHeader(fid);
    
    dataStruct.Version = FileHeader.systemID;
    dataStruct.SubjectID = '';
    dataStruct.Comment = char(SMR.FileHeader.fileComment);
    dataStruct.StartDate = [];
    dataStruct.FileFormat = '.smr';
    dataStruct.freq = SMR.FileHeader.usPerTime * SMR.FileHeader.timePerADC * 0.000001; %microseconds
    dataStruct.nSeconds = [];
    dataStruct.nChannels = [];
    ...
    for n = 1:length(ChannelNames)
        if ChannelNames(n).kind == 1 %if data channel
            ...
            if any(strcmpi(params.DataChannels,'ALL')) ||  any(~cellfun(@isempty,ChanCellArray))
                ...
                dataIDX = dataIDX +1;
                dataStruct.Channel{dataIDX,1}.Header = ChannelHeader;
                dataStruct.Channel{dataIDX,1}.data = ChannelData;
                ...
                dataStruct.Channel{dataIDX,1}.time = timebase;
        ...
        elseif any(strcmpi(params.EventChannel,'ALL')) || strcmpi(ChannelNames(n).title,params.EventChannel) % if there is a keyboard / digimark channel
            ...
            for m = 1:length(DigiMarks)  %this assumes only the first char is used in DigiMark
                ...
                marker{markerIDX,1}.name = DigiMarks(m);
                marker{markerIDX,1}.TimeStamp = ChannelData.timings(DigiMarkIndex);
            ...
            dataIDX = dataIDX +1;
            dataStruct.Channel{dataIDX,1}.Name
            dataStruct.Channel{dataIDX,1} = marker;
        end
    end
